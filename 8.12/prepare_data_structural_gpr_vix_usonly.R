# =============================================================================
# GPR + VIX GVAR data preparation: GPR endogenous ONLY in the US/dominant block
#
# Global endogenous ordering:
#   US:     GPR, VIX, y, dp, r, de, deq
#   non-US: y, dp, r, de, deq
#
# Foreign/global block:
#   US:
#     foreign_y, foreign_dp, foreign_r, foreign_de, foreign_deq
#
#   non-US:
#     foreign_y, foreign_dp, foreign_r, foreign_de, foreign_deq, global_vix
#
# IMPORTANT:
#   global_gpr is deliberately NOT included in any non-US local equation,
#   contemporaneously or with lags.
#
# Hence GPR remains an endogenous variable of the US/dominant block, while its
# international transmission must operate through VIX, US macro-financial
# variables and the ordinary foreign-macro GVAR channels.
# =============================================================================

prepare_gvar_data <- function(
    data_file = "data/model_input.csv",
  ...
  # Global variable dimension remains 72:
  #   US has 7 endogenous variables; 13 non-US economies have 5 each.
  K <- ncol(xglobal)

  endogenous_total <- sum(
    vapply(
      countries,
      function(cc) if (cc == "US") 7L else 5L,
      integer(1)
    )
  )

  if (endogenous_total != K) {
    stop(
  ...
  if (K != 72L) {
    stop(
      "GPR+VIX US-only experiment should contain exactly 72 global variables."
    )
  }

  gpr_idx <- match("US_gpr", colnames(xglobal))
  vix_idx <- match("US_vix", colnames(xglobal))

  if (is.na(gpr_idx) || is.na(vix_idx)) {
    stop("US_gpr or US_vix not found in global vector.")
  }

  # gW[[i]] maps the 72-variable global vector into each local system.
  #
  # US rows = 12:
  ...
  #    global_vix]
  #
  # There is NO global_gpr row for non-US countries.
  ...
  for (i in seq_along(countries)) {
    ...
    if (cc != "US") {
      ...
      # VIX remains a global financial-risk channel for non-US economies.
      vix_row <- matrix(
        0,
        nrow = 1,
        ncol = K
  ...
  if (!identical(
    as.integer(local_rows),
    as.integer(expected_rows)
  )) {
    stop(
      "Unexpected local mapping dimensions. Found: ",
      paste(
        names(local_rows),
        local_rows,
        sep = "=",
  ...
  # Structural assertion: no non-US W matrix may contain a row selecting US_gpr.
  for (i in seq_along(countries)) {
    cc <- countries[[i]]
    if (cc == "US") next
    ...
    if ("global_gpr" %in% rownames(Wi)) {
      stop(
        "Specification violation: global_gpr row still exists for ",
        cc
      )
    }

    # No foreign/global row may load on US_gpr.
    k_i <- 5L
    foreign_part <- Wi[(k_i + 1L):nrow(Wi), , drop = FALSE]

    if (any(abs(foreign_part[, gpr_idx]) > 1e-14)) {
      stop(
        "Specification violation: non-US mapping still loads on US_gpr for ",
        cc
      )
    }
  }
  ...
  list(
    ...
    specification = "GPR endogenous in US only; VIX global for non-US"
  )
}
