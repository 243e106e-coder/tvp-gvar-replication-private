...
summary_lines <- c(
  ...
  paste0("Trade-weight method: ", weight_method),
  paste0("Trade-weight format: ", toupper(tools::file_ext(weight_file))),
  ...
  paste0(
    ...
    sprintf("%.12f", min(raw_rs)), " - ", sprintf("%.12f", max(raw_rs))
  ),
  ...
  paste0(
    ...
    sprintf("%.12f", min(rowSums(W))), " - ", sprintf("%.12f", max(rowSums(W))
  )
)
