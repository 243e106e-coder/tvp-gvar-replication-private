...
# =============================================================================
# RECOVERY ONLY: resume dominant-unit GPR+VIX post-estimation from a saved
# posterior.  This script DOES NOT run MCMC.
#
# Designed for failed source run 32576318744, where all 15 unit estimations
# completed and results/predDens_dominant_gpr_vix.rda was saved before the
# p=2,q=1 IRF reconstruction dimension error.
# =============================================================================
...
out_dir <- Sys.getenv("TVPGVAR_RECOVERY_RESULTS", "results")
...
if (!file.exists(posterior_file)) stop("Missing saved posterior: ", posterior_file)
if (!is.finite(nhor) || nhor < 1L) stop("Invalid IRF horizon.")
if (!is.finite(shock_pct) || shock_pct <= 0) stop("Invalid GPR shock percent.")

dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
if (normalizePath(out_dir, mustWork = FALSE) != normalizePath("results", mustWork = FALSE)) {
  stop("Recovery script currently expects TVPGVAR_RECOVERY_RESULTS=results.")
}
...
if (is.null(xglobal) || is.null(gW) || is.null(cN)) {
  stop("Data.setup is incomplete in saved posterior.")
}

# Infer p from the actual estimated ALPHA labels instead of trusting a workflow
# input.  This prevents a p=1/p=2 recovery mismatch.
...
if (!length(lag_nums0) || anyNA(lag_nums0)) stop("Cannot infer domestic lag order.")
lag_order <- max(lag_nums0)

cat("============================================================\n")
cat(" DOMINANT-UNIT POSTERIOR RECOVERY (NO MCMC)\n")
cat(" Posterior: ", posterior_file, "\n", sep = "")
cat(" Units: ", length(cN), "\n", sep = "")
cat(" Inferred p: ", lag_order, "\n", sep = "")
cat(" Foreign/global q: 1 for country blocks\n")
cat(" Horizon: ", nhor, "\n", sep = "")
cat(" GPR shock: +", shock_pct, "%\n", sep = "")
cat("============================================================\n\n")

# Re-copy source-run diagnostics if the recovery workflow made them available.
prior_results <- dirname(posterior_file)
for (f in c(
  "build_structural_input_summary.txt",
  "dominant_design_preflight.csv",
  "dominant_lag_spec_diagnostic.csv",
  "dominant_unit_mapping_audit.csv",
  "residual_serial_correlation_diagnostic.csv",
  "vix_input_summary.txt"
)) {
  src <- file.path(prior_results, f)
  dst <- file.path("results", f)
  if (file.exists(src) && normalizePath(src, mustWork = FALSE) != normalizePath(dst, mustWork = FALSE)) {
...
cat("\nDominant-unit posterior recovery complete (NO MCMC).\n")
cat("Global stable share: ", mean(stable_mask), "\n", sep="")

writeLines(c(
  paste0("source_posterior=", posterior_file),
  paste0("inferred_p=", lag_order),
  "foreign_global_q=1",
  paste0("horizon=", nhor),
  paste0("shock_pct=", shock_pct),
  "mcmc_rerun=FALSE",
  "fix=p2_q1_missing_foreign_lag2_padded_with_structural_zeros"
), "results/dominant_recovery_metadata.txt")
