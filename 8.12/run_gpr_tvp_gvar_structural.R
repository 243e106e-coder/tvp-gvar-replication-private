...
# Fast preflight: validate the exact p/q design used by every country before
# starting the expensive MCMC.  Also force a singular contemporaneous matrix
# through the repaired inverse path so the old ginv(try-error) bug cannot hide
# until posterior reconstruction.
if (!exists("tvpgvar_safe_inverse", mode = "function")) {
  stop("Generated BVAR estimator is missing tvpgvar_safe_inverse().")
}
singular_probe <- matrix(c(1, 2, 2, 4), nrow = 2L)
singular_probe_inverse <- tvpgvar_safe_inverse(
  singular_probe,
  context = "preflight singular-matrix probe"
)
if (!is.matrix(singular_probe_inverse) ||
    !is.numeric(singular_probe_inverse) ||
    any(!is.finite(singular_probe_inverse))) {
  stop("Singular-matrix inverse preflight failed.")
}
...
design_preflight <- do.call(rbind, lapply(seq_along(cN), function(i) {
  ...
  weighted <- Wi %*% t(as.matrix(xglobal))
  Wex <- t(weighted[(ncol(End) + 1L):nrow(weighted), , drop = FALSE])
  storage.mode(Wex) <- "double"

  Ylag <- mlag(End, lag_order)
  Wexlag <- tvpgvar_wex_lag(Wex, 1L, cc, gpr_lag_mode)
  Xcheck <- cbind(Wex, Wexlag, Ylag)
  Xcheck <- Xcheck[(lag_order + 1L):nrow(Xcheck), , drop = FALSE]
  Xcheck <- cbind(constant = 1, Xcheck)
  Ycheck <- End[(lag_order + 1L):nrow(End), , drop = FALSE]
  storage.mode(Xcheck) <- "double"
  storage.mode(Ycheck) <- "double"

  if (nrow(Xcheck) != nrow(Ycheck) ||
      any(!is.finite(Xcheck)) || any(!is.finite(Ycheck))) {
    stop("P2 design preflight failed for ", cc,
         ": X=", paste(dim(Xcheck), collapse = "x"),
         ", Y=", paste(dim(Ycheck), collapse = "x"))
  }
  ...
  data.frame(
    ...
    p = lag_order,
    q = 1L,
    observations = nrow(Xcheck),
    regressors = ncol(Xcheck),
    equations = ncol(Ycheck),
    design_rank = qr(Xcheck)$rank,
    full_column_rank = qr(Xcheck)$rank == ncol(Xcheck),
    stringsAsFactors = FALSE
  )
}))
write.csv(design_preflight,
          "results/p2_design_preflight.csv",
          row.names = FALSE)
cat("P2 design and singular-inverse preflight passed for ",
    nrow(design_preflight), " country blocks.\n", sep = "")
...
sfExport(list = list(
  ...
  "gpr_lag_mode", "tvpgvar_wex_lag", "tvpgvar_safe_inverse", "rng_streams"
))
...
predDens <- sfLapply(seq_along(cN), function(i) {
  ...
  tryCatch(
    ...
    error = function(e) {
      stop("Country block ", cN[[i]], " failed: ", conditionMessage(e),
           call. = FALSE)
    }
  )
})
